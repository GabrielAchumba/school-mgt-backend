...
import (
	...
	"github.com/GabrielAchumba/school-mgt-backend/realestate/user-module/models"
...
func New(mongoClient *mongo.Client, config config.Settings, ctx context.Context,
	tokenMaker token.Maker) UserService {
	collection := mongoClient.Database(config.Database.DatabaseName).Collection(config.TableNames.RealEstateUser)
...
func (impl serviceImpl) SeedAdmin() {
	admin := dtos.CreateUserRequest{
		Password:      "real*012023",
		FirstName:     "admin",
		LastName:      "admin",
		PhoneNumber:   "07032488605",
		CountryCode:   "+234",
		UserName:      "real@estate.com",
		UserType:      "Admin",
		DesignationId: "CEO",
	}
...
func (impl serviceImpl) LoginUser(requestModel dtos.LoginUserRequest) (interface{}, error) {
	...
	rsp := dtos.LoginUserResponse{
		...
		User: dtos.UserResponse{
			...
			RealestateCompany: modelDto.RealestateCompany,
		},
	}
...
func (impl serviceImpl) DeleteUser(id string, schoolId string) (int64, error) {
	...
	if result.DeletedCount < 1 {
		return result.DeletedCount, errors.Error("User with specified ID not found!")
	}
...
func (impl serviceImpl) PutUser(id string, User dtos.UpdateUserRequest) (interface{}, error) {
	...
	update := bson.D{bson.E{Key: "designationyd", Value: updatedUser.DesignationId},
		...
		bson.E{Key: "realestatecompany", Value: updatedUser.RealestateCompany}}
